...
class LoginForm(FlaskForm):
    ...
    login = StringField(validators=[InputRequired(),
                                    Length(min=5)], render_kw={"placeholder": "Dante"})
...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(validators=[InputRequired(),
                                         ...
                                         EqualTo('confirm_password',
                                                 message='Les mots de passe doivent être les memes')],
                             render_kw={"placeholder": PLACEHOLDER})

    confirm_password = PasswordField(validators=[InputRequired(), Length(min=5)],
                                     render_kw={"placeholder": PLACEHOLDER})
