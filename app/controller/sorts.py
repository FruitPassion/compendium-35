...
def resultat_recherche():
    ...
    return render_template("sorts/liste.html", sorts=sorts, nbr_sorts=len(sorts), page=1, nb_pages=1)
...
def liste(page):
    ...
    return render_template("sorts/liste.html", sorts=sorts, nbr_sorts=nbr_sorts, page=page, nb_pages=nb_pages)
...
@sorts.route("/branche/<string:ecole>/<string:branche>/", defaults={'page': 1}, methods=["GET"])
...
def branche(ecole, branche, page):
    ...
    nb_pages = int(nbr_sorts/sort_par_page+1)
    if nb_pages < page:
        return redirect(url_for("sorts.branche", ecole=ecole, brancher=branche, page=nb_pages))
    elif page < 1:
