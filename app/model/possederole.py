...
class PossederRole(db.Model):
    ...
    id_compte = db.Column(
        db.ForeignKey(f"{DB_SCHEMA}.compte.id_compte"), primary_key=True, nullable=False
    )
    id_role = db.Column(
        db.ForeignKey(f"{DB_SCHEMA}.role.id_role"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    ...
    compte = db.relationship(
        "Compte", primaryjoin="PossederRole.id_compte == Compte.id_compte"
    )
