...
from model.shared_model import DB_SCHEMA, Source, db
...
class Sort(db.Model):
    ...
    def get_sorts(page=1, per_page=50):
        ...
        """
        return (
            db.session.query(Sort, Source)
            .join(Source)
            .order_by(Sort.nom.asc())
    ...
    def get_liste_sort_par_nom(sort_nom):
        ...
        return (
            db.session.query(Sort, Source)
            .join(Source)
            .filter(Sort.nom.like(f"%{sort_nom}%"))
            .all()
        )
    ...
    def get_recherche_sorts_par_source():
        ...
        """
        return (
            db.session.query(Source.nom, db.func.count(Sort.id_sort))
            .join(Sort)
            .group_by(Source.nom)
            .all()
        )
    ...
    def get_liste_sorts_par_source(source, page=1, per_page=50):
        ...
        return (
            db.session.query(Sort, Source)
            .join(Source)
            .filter(Sort.id_source == Source.id_source)
            .filter(Source.nom == source)
            .order_by(Sort.nom.asc())
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
    ...
    def get_recherche_sorts_par_ecole():
        ...
        """
        return (
            db.session.query(Sort.ecole, Sort.branche)
    ...
    def get_number_of_sorts_par_ecole_branche(ecole, branche):
        ...
        """
        return (
            db.session.query(Sort)
            .filter(Sort.ecole.like(f"%{ecole}%"))
            .filter(Sort.branche.like(f"%{branche}%"))
            .count()
        )
    ...
    def get_recherche_sorts_par_registre():
        ...
        """
        return (
            db.session.query(distinct(Sort.registre))
    ...
    def get_number_of_sorts_par_registre(registre):
        ...
        """
        return (
            db.session.query(Sort).filter(Sort.registre.like(f"%{registre}%")).count()
        )
    ...
    def get_liste_sorts_par_registre(registre, page=1, per_page=50):
        ...
        return (
            db.session.query(Sort, Source)
            .join(Source)
            .filter(Sort.registre.like(f"%{registre}%"))
            .order_by(Sort.nom.asc())
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
...
class DonnerSort(db.Model):
    ...
    id_sort = db.Column(
        db.ForeignKey(f"{DB_SCHEMA}.sort.id_sort"),
        primary_key=True,
        nullable=False,
        index=True,
    )
    ...
    capacite = db.relationship(
        "Capacite", primaryjoin="DonnerSort.id_capacite == Capacite.id_capacite"
    )
...
class AccederSort(db.Model):
    ...
    capacite_sort = db.relationship(
        "CapaciteSort",
        primaryjoin="AccederSort.id_capacite_sort == CapaciteSort.id_capacite_sort",
    )
...
class ContenirSort(db.Model):
    ...
    id_sort = db.Column(
        db.ForeignKey(f"{DB_SCHEMA}.sort.id_sort"), primary_key=True, nullable=False
    )
    ...
    domaine = db.relationship(
        "Domaine", primaryjoin="ContenirSort.id_domaine == Domaine.id_domaine"
    )
